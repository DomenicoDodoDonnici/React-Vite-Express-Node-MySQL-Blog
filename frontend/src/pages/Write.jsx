...
export default function Write() {
    ...
    const fetchData = async () => {
      try {
        const { data } = await axios.get(
          `http://localhost:8000/api/posts/${state}`
        );
  ...
  const handleClick = async (e) => {
    e.preventDefault();
    if ((!file && !defaultFile) || !category || !title || !value)
      return console.log("nikal");
      ...
      <div className="menu">
        <div className="item">
          ...
          <div className="buttons">
            <button>Salva bozza</button>
            <button onClick={handleClick}>Pubblica</button>
          </div>
        </div>
        <div className="item">
          <h1>Categoria</h1>
          ...
          <div className="cat">
            <input
              ...
              onChange={(e) => setCategory(e.target.value)}
            />
            <label htmlFor="science">Scienza</label>
          </div>
          <div className="cat">
            <input
              type="radio"
